...
class TestGenerateNarrativeVerdict:
	def test_returns_structured_data(self):
		with patch(
			"claude_candidate.generator.call_claude", return_value=SAMPLE_NARRATIVE_RESPONSE
		):
...
class TestPerTypeTimeouts:
	...
	def test_generate_cover_letter_passes_correct_timeout(self, minimal_assessment):
		...
		with patch(
			"claude_candidate.generator.call_claude", return_value="Dear Hiring Manager"
		) as mock_call:
	...
	def test_generate_interview_prep_passes_correct_timeout(self, minimal_assessment):
		...
		with patch(
			"claude_candidate.generator.call_claude", return_value="## Technical Topics"
		) as mock_call:
