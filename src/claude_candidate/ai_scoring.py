...
def score_orchestration(messages: list["NormalizedMessage"]) -> float:
    ...
    found = sum(
        [uses_agent, has_subagent_diversity, has_task_lifecycle, has_parallel, has_team, has_messaging]
    )
...
def score_prompt_engineering(messages: list["NormalizedMessage"]) -> float:
    ...
    has_constraints = bool(
        re.search(r"\b(must|never|always|do not)\b", all_text, re.IGNORECASE)
    )
    ...
    has_role_spec = bool(
        re.search(r"\b(you are|act as|your role)\b", all_text, re.IGNORECASE)
    )
...
def score_tool_ecosystem(messages: list["NormalizedMessage"]) -> float:
    ...
    has_browser = any(
        "playwright" in n.lower() or "browser" in n.lower()
        for n in tool_names
    )

    # Category 3: GitHub API (gh CLI in bash commands or gh-related tool names)
    has_github = any(
        re.search(r"\bgh\b", cmd) for cmd in bash_commands
    ) or any(
    ...
    distinct_non_standard = len({
        n for n in tool_names if n not in STANDARD_TOOLS
    })
...
def score_error_recovery(messages: list["NormalizedMessage"]) -> float:
    ...
    has_acknowledgment = bool(
        re.search(r"\b(error|failed|fail|fix|retry|incorrect|wrong|broken)\b", all_text, re.IGNORECASE)
    )
