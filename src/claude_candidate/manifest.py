...
def verify_manifest(manifest: SessionManifest) -> dict:
    ...
    if stored_hash and stored_hash != computed_hash:
        errors.append(f"Manifest hash mismatch: stored={stored_hash[:16]}... computed={computed_hash[:16]}...")
    ...
    if stats.total_sessions != len(manifest.sessions):
        errors.append(f"Session count mismatch: stats={stats.total_sessions} actual={len(manifest.sessions)}")
