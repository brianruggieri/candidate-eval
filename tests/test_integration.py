...
class TestEndToEndFlow:
	...
	def test_complete_pipeline(self, fixtures_dir, tmp_path):
		...
		rp = ResumeProfile.from_json((fixtures_dir / "sample_resume_profile.json").read_text())
		...
		reqs = [
			QuickRequirement(**r)
			for r in json.loads((fixtures_dir / "sample_job_posting.requirements.json").read_text())
		]
...
class TestSessionsScanWhitelist:
	...
	def test_scan_session_dir_skips_whitelist(self, tmp_path):
		...
		projects_dir = self._make_sessions_dir(
			tmp_path,
			{
		...
		result = runner.invoke(
			...
			[
				...
				"scan",
				"--session-dir",
				str(projects_dir / "-Users-u-git-proj-a"),
				"--output",
				str(output_path),
			],
		)
	...
	def test_scan_accept_defaults_without_whitelist_errors(self, tmp_path, monkeypatch):
		...
		assert result.exit_code != 0
		assert "No whitelist found" in result.output or "No whitelist found" in (
