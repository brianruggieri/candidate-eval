...
@click.option(
	"--job", "-j", type=click.Path(exists=True), required=True, help="Path to job posting text file"
)
...
def assess(
	...
	click.echo(
		f"  Merged: {merged.corroborated_skill_count} corroborated, "
...
@click.option("--output", "-o", type=click.Path(), help="Output path for proof package markdown")
...
@click.option("--output", "-o", type=click.Path(), help="Output path for generated deliverable")
...
def generate_site(shortlist_id: int, output_dir: str, deploy: bool, db: str | None) -> None:
	...
	if assessment_record is None:
		click.echo(
			f"No assessment found for shortlist ID {shortlist_id}. Run an assessment first.",
			err=True,
		)
		raise SystemExit(1)
	...
	top_matches = [m for m in skill_matches if m.get("match_status") in positive_statuses][:3]
...
def shortlist(db: str | None) -> None:
	...
	# Print table header
	click.echo(
		f"{'Grade':<6} {'Company':<20} {'Title':<30} {'Location':<15} {'Salary':<15} {'Added':<12}"
	)
	click.echo("-" * 100)
	for item in items:
		click.echo(
...
@click.option(
	"--profile", "-p", type=click.Path(exists=True), help="CandidateProfile JSON for correlation"
)
...
@click.option(
	"--output", "-o", type=click.Path(), default=None, help="Output path for curated profile"
)
@click.option("--skip-gaps", is_flag=True, default=False, help="Skip scenario gap-fill questions")
def profile_review(candidate: str, output: str | None, skip_gaps: bool) -> None:
	...
	click.echo(
		f"  {cp.session_count} sessions analysed  |  "
	...
	if cp.problem_solving_patterns:
		...
		for pattern in cp.problem_solving_patterns:
			...
			click.echo(
				f'    "{pattern.description[:100]}{"..." if len(pattern.description) > 100 else ""}"'
			)
	...
	adjusted_count = sum(
		1 for p in curated_patterns if p["source"] == "session_evidence" and p["delta"] != 0
	)
...
def resume_onboard(resume_path: str, output: str | None, accept_defaults: bool) -> None:
	...
	if not accept_defaults:
		click.echo(
			f"Rate each skill: {DEPTH_LABELS}, Enter=accept default, duration for deep/expert only.\n"
		)
	...
	click.echo(
		f"\n{len(curated_skills)} skills: "
		+ ", ".join(f"{v} {k}" for k, v in sorted(depths.items(), key=lambda x: -x[1]))
	)
	...
	output_path = (
		Path(output) if output else Path.home() / ".claude-candidate" / "curated_resume.json"
	)
...
@sessions.command()
@click.option(
	"--session-dir", type=click.Path(exists=True), help="Directory containing session JSONL files"
)
@click.option("--output", "-o", type=click.Path(), help="Output path for CandidateProfile JSON")
@click.option(
	"--reselect", is_flag=True, default=False, help="Re-select projects for whitelist interactively"
)
@click.option(
	"--accept-defaults",
	is_flag=True,
...
def scan(
	...
	if not no_compact:
		...
		click.echo(
			f"  Evidence compacted (compaction_version={profile.compaction_version}, "
