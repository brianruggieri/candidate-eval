...
def _summary_section(assessment: FitAssessment) -> str:
    ...
    verdict = VERDICT_LABELS.get(
        assessment.should_apply, assessment.should_apply,
    )
    lines = [
        "## Assessment Summary",
        "",
...
def _skill_row(skill: SkillMatchDetail) -> str:
    ...
    source_label = EVIDENCE_SOURCE_LABELS.get(
        skill.evidence_source.value, skill.evidence_source.value,
    )
...
def _matrix_from_assessment(assessment: FitAssessment) -> str:
    ...
    corroborated = sum(
        1 for s in assessment.skill_matches
        if s.evidence_source.value == "corroborated"
    )
    sessions_only = sum(
        1 for s in assessment.skill_matches
        if s.evidence_source.value == "sessions_only"
    )
    resume_only = sum(
        1 for s in assessment.skill_matches
        if s.evidence_source.value == "resume_only"
    )
    return (
