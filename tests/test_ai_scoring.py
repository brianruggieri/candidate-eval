...
class TestScoreOrchestration:
    ...
    def test_agent_usage_raises_score(self) -> None:
        messages = [
            assistant_msg(tool_use_block("Agent", {"prompt": "do something", "subagent_type": "general"})),
...
class TestScoreAiDomain:
    ...
    def test_api_call_patterns_raise_score(self) -> None:
        messages = [
            assistant_msg(text_block("response = client.messages.create(model='claude-3-5-sonnet')")),
        ]
        score = score_ai_domain(messages)
        assert score > 0.0
    ...
    def test_rag_patterns_raise_score(self) -> None:
        messages = [
            assistant_msg(text_block("Use embedding vectors for retrieval. Chunk the documents first.")),
        ]
        score = score_ai_domain(messages)
        assert score > 0.0
    ...
    def test_eval_patterns_raise_score(self) -> None:
        messages = [
            assistant_msg(text_block("We need to evaluate the model output using benchmark metrics and score it.")),
        ]
        score = score_ai_domain(messages)
        assert score > 0.0
    ...
    def test_structured_output_patterns_raise_score(self) -> None:
        messages = [
            assistant_msg(text_block("Use response_model=MySchema and tool_use with json_schema validation.")),
        ]
        score = score_ai_domain(messages)
        assert score > 0.0
...
class TestScorePromptEngineering:
    ...
    def test_constraint_language_raises_score(self) -> None:
        messages = [
            user_msg("You must never modify tests. Always run the linter first. Do not edit config files."),
        ]
        score = score_prompt_engineering(messages)
        assert score > 0.0
    ...
    def test_output_format_spec_raises_score(self) -> None:
        messages = [
            user_msg("Respond in JSON. Format as a list. Return a dict with keys 'name' and 'score'."),
        ]
        score = score_prompt_engineering(messages)
        assert score > 0.0
    ...
    def test_role_specification_raises_score(self) -> None:
        messages = [
            user_msg("You are an expert Python developer. Act as a senior engineer. Your role is to review code."),
        ]
        score = score_prompt_engineering(messages)
        assert score > 0.0
...
class TestScoreToolEcosystem:
    ...
    def test_only_standard_tools_scores_low(self) -> None:
        messages = [
            ...
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "x", "new_string": "y"})),
    ...
    def test_mcp_tools_raise_score(self) -> None:
        messages = [
            assistant_msg(tool_use_block("mcp__playwright__browser_navigate", {"url": "http://example.com"})),
        ]
        score = score_tool_ecosystem(messages)
        assert score > 0.0
    ...
    def test_browser_automation_raises_score(self) -> None:
        messages = [
            assistant_msg(tool_use_block("mcp__plugin_playwright_playwright__browser_snapshot", {})),
    ...
    def test_github_api_raises_score(self) -> None:
        messages = [
            assistant_msg(tool_use_block("Bash", {"command": "gh pr create --title 'fix' --body 'desc'"})),
        ]
        score = score_tool_ecosystem(messages)
        assert score > 0.0
    ...
    def test_diverse_tool_ecosystem_scores_high(self) -> None:
        messages = [
            assistant_msg(tool_use_block("mcp__context7__resolve_library_id", {"library": "fastapi"})),
            assistant_msg(tool_use_block("mcp__plugin_playwright_playwright__browser_navigate", {"url": "http://localhost"})),
...
class TestScoreErrorRecovery:
    ...
    def test_no_recovery_patterns_scores_low(self) -> None:
        messages = [
            assistant_msg(text_block("Let me implement this."), tool_use_block("Write", {"file_path": "/foo.py", "content": "code"})),
    ...
    def test_diagnostic_before_fix_raises_score(self) -> None:
        messages = [
            ...
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "broken", "new_string": "fixed"})),
            user_msg_with_result("ok"),
        ]
        score = score_error_recovery(messages)
        assert score > 0.0
    ...
    def test_error_acknowledgment_raises_score(self) -> None:
        messages = [
            assistant_msg(text_block("I see an error here. The test failed. Let me fix the retry logic.")),
        ]
        score = score_error_recovery(messages)
        assert score > 0.0
    ...
    def test_iterative_refinement_raises_score(self) -> None:
        messages = [
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "a", "new_string": "b"})),
            user_msg_with_result("ok"),
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "b", "new_string": "c"})),
            user_msg_with_result("ok"),
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "c", "new_string": "d"})),
            user_msg_with_result("ok"),
        ]
        score = score_error_recovery(messages)
        assert score > 0.0
    ...
    def test_full_recovery_session_scores_high(self) -> None:
        messages = [
            ...
            assistant_msg(
                ...
                tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "bad", "new_string": "good"}),
            ),
            user_msg_with_result("ok"),
            # Test-fix cycle
            assistant_msg(tool_use_block("Bash", {"command": "pytest tests/"})),
            user_msg_with_result("FAILED"),
            assistant_msg(tool_use_block("Edit", {"file_path": "/foo.py", "old_string": "good", "new_string": "better"})),
            user_msg_with_result("ok"),
            assistant_msg(tool_use_block("Bash", {"command": "pytest tests/"})),
            user_msg_with_result("passed"),
        ]
...
class TestComputeAiEngineeringScore:
    ...
    def test_level_expert_threshold(self) -> None:
        ...
        messages = [
            ...
            assistant_msg(tool_use_block("Edit", {"file_path": "/rag.py", "old_string": "bad", "new_string": "good"})),
            user_msg_with_result("ok"),
            assistant_msg(tool_use_block("Bash", {"command": "pytest tests/"})),
            user_msg_with_result("passed"),
        ]
    ...
    def test_all_scores_in_range(self) -> None:
        ...
        result = compute_ai_engineering_score(messages)
        for key in ("orchestration", "ai_domain", "prompt_engineering", "tool_ecosystem", "error_recovery", "composite"):
