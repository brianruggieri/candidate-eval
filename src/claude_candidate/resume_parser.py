...
LOCATION_PATTERN = re.compile(r"(?<![a-z])([A-Z][a-z]+(?:\s[A-Z][a-z]+)?),\s*([A-Z]{2})(?:\s|$|\))")

# Location at end of line (for header name/location splitting)
LOCATION_EOL = re.compile(r"([A-Z][a-z]+(?:\s[A-Z][a-z]+)?),\s*([A-Z]{2})\s*$")
		...
		if role_match:
			...
			for j in range(i + 1, min(i + 4, len(lines))):
				...
				if dm:
					...
					raw_end = dm.group(2)
					end_date = (
						None if raw_end.lower() in ("present", "current") else _parse_date(raw_end)
					)
					date_line_idx = j
					break
...
def parse_resume_text(
	...
	def _recency_for_skill(
		skill: str,
	) -> Literal["current_role", "previous_role", "historical", "unknown"]:
