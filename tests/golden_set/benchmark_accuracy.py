...
def run_benchmark():
	...
	if graded:
		print(
			f"Exact match: {exact}/{len(graded)} | Within 1 grade: {within_1}/{len(graded)} | Off by 2+: {off_by_2}/{len(graded)}"
		)
	...
	for slug, r in sorted(results.items(), key=lambda x: x[1]["score"], reverse=True):
		...
		print(
			f"  {slug:.<55} actual={r['actual']:>3} expected={exp_str:>3} {delta_str} score={r['score']}%"
		)
	...
	if graded:
		...
		for slug, r in worst:
			if abs(r["delta"]) > 1:
				print(
					f"  {slug:.<55} actual={r['actual']:>3} expected={r['expected']:>3} delta={r['delta']:+d} score={r['score']}%"
				)
