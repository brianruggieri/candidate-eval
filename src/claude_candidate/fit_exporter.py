...
def select_evidence_highlights(
	...
	*,
	...
	strong = [m for m in skill_matches if m.get("match_status") in ("strong_match", "exceeds")]
	strong.sort(
		key=lambda m: (
...
def export_fit_assessment(
	...
	*,
	...
	if len(enriched_matches) < 3:
		threshold_errors.append(f"Skill matches: {len(enriched_matches)} found, minimum 3 required")
	if len(projects) < 1:
		threshold_errors.append("Projects: 0 found, minimum 1 required")
	if threshold_errors:
		raise ValueError(
