...
class SkillMatch(BaseModel):
	...
	match_status: Literal["strong_match", "partial_match", "adjacent", "no_evidence", "exceeds"]
...
class MatchEvaluation(BaseModel):
	...
	def must_have_coverage(self) -> tuple[int, int]:
		...
		must_haves = [
			m for m in self.skill_matches if m.requirement.priority == RequirementPriority.MUST_HAVE
		]
		met = sum(
			1 for m in must_haves if m.match_status in ("strong_match", "exceeds", "partial_match")
		)
