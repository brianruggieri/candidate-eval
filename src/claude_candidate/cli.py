...
async def _fetch_all_pages(urls: list[str]) -> dict[str, str | None]:
    """Fetch all LinkedIn pages using a single persistent Playwright browser session.

    Uses a persistent browser profile at ~/.claude-candidate/browser_profile/
    so that LinkedIn login persists between runs. The browser opens visibly on
    the first run so the user can log in; subsequent runs reuse the session.
    """
    from playwright.async_api import async_playwright

    profile_dir = Path.home() / ".claude-candidate" / "browser_profile"
...
async def _fetch_tracker_urls(tracker_url: str) -> list[str]:
    """Open the LinkedIn job tracker page and scrape all job posting links.

    Paginates through all pages and collects URLs matching the LinkedIn
    job view pattern (e.g. /jobs/view/12345/).
    """
    import re
    from playwright.async_api import async_playwright
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto(tracker_url, wait_until="domcontentloaded", timeout=30000)
            await page.wait_for_timeout(3000)

            # Collect job links from the page, scrolling to load more
            seen: set[str] = set()
            last_count = 0
            ...
            for _ in range(max_scroll_attempts):
                ...
                for link in links:
                    # Normalize to just the /jobs/view/{id}/ part
                ...
                if len(seen) == last_count:
                    break  # No new links found after scrolling
                last_count = len(seen)
...
@main.command("batch-assess")
@click.argument("urls_file", type=click.Path(exists=True), required=False, default=None)
@click.option("--server", default="http://localhost:7429", help="Server URL")
@click.option(
    "--from-tracker", is_flag=True, default=False,
    help="Scrape URLs from LinkedIn job tracker page instead of a file",
)
@click.option(
    "--tracker-url",
    default="https://www.linkedin.com/jobs/tracker/",
    help="LinkedIn job tracker URL (used with --from-tracker)",
)
def batch_assess(
    urls_file: str | None,
    server: str,
    ...
    """Batch assess jobs from a file of LinkedIn URLs (one per line).

    Provide a file with one LinkedIn job URL per line, or use --from-tracker
    to automatically scrape URLs from your LinkedIn job tracker page.

    Requires Playwright for page fetching. Install with:

    \b
        pip install playwright && playwright install chromium

    The browser profile is persisted at ~/.claude-candidate/browser_profile/
    so LinkedIn login carries across runs.
    """
    import asyncio
    ...
    try:
        ...
        click.echo(
            "httpx is required for batch-assess. Install with: pip install httpx",
            err=True,
        )
        return
    ...
    try:
        ...
        click.echo(
            "Playwright is required for batch-assess. Install with:\n"
            "  pip install playwright && playwright install chromium",
            err=True,
        )
        return
    ...
    # Gather URLs
    if from_tracker:
        click.echo(f"Scraping job URLs from tracker: {tracker_url}")
        urls = asyncio.run(_fetch_tracker_urls(tracker_url))
        if not urls:
            click.echo("No job URLs found on the tracker page.")
            return
        click.echo(f"Found {len(urls)} job URLs from tracker.")
    elif urls_file:
        urls = [
            line.strip()
        ...
        if not urls:
            ...
            return
        click.echo(f"Found {len(urls)} job URLs in file.")
    else:
        click.echo("Provide a URLs file or use --from-tracker.", err=True)
        return

    # Step 1: Fetch all pages with a single browser session
    click.echo("\nFetching pages...")
    page_texts = asyncio.run(_fetch_all_pages(urls))

    fetched = sum(1 for v in page_texts.values() if v)
    click.echo(f"Fetched {fetched}/{len(urls)} pages.\n")

    if fetched == 0:
        click.echo("Could not fetch any pages. Check your LinkedIn login.")
        return

    # Step 2: Extract + assess each page via the server
    results: list[dict] = []
    ...
    async def _process_all() -> None:
        async with httpx.AsyncClient(timeout=60) as client:
            for i, url in enumerate(urls):
                ...
                if not page_text:
                    click.echo(f"  [{i+1}/{len(urls)}] {url[:60]}... SKIP (no page text)")
                    continue

                click.echo(f"  [{i+1}/{len(urls)}] {url[:60]}...")

                # Extract posting
                try:
                    resp = await client.post(
                        f"{server}/api/extract-posting",
                        json={"url": url, "title": "", "text": page_text[:15000]},
                    )
                    if resp.status_code != 200:
                        click.echo(f"    Extraction failed: HTTP {resp.status_code}")
                        continue
                    posting = resp.json()
                ...
                try:
                    resp = await client.post(
                        f"{server}/api/assess/partial",
                        json={
                            "posting_text": posting.get("description", ""),
                            "company": posting.get("company", "Unknown"),
                            "title": posting.get("title", "Unknown"),
                            "posting_url": url,
                            "requirements": posting.get("requirements"),
                            "seniority": posting.get("seniority") or "unknown",
                        },
                    )
                    if resp.status_code != 200:
                        click.echo(f"    Assessment failed: HTTP {resp.status_code}")
                        continue
                    assessment = resp.json()
                except Exception as e:
                    click.echo(f"    Assessment error: {e}")
                    continue

                pct = assessment.get("partial_percentage") or (assessment.get("overall_score", 0) * 100)
                grade = assessment.get("overall_grade", "?")
                comp = posting.get("company", "?")
                ttl = posting.get("title", "?")
                click.echo(f"    -> {comp} | {ttl} | {pct:.0f}% {grade}")

                skill_match = assessment.get("skill_match")
                results.append({
                    "url": url,
                    "company": posting.get("company", "Unknown"),
                    "title": posting.get("title", "Unknown"),
                    ...
                    "percentage": pct,
                    "grade": grade,
                    "skill_score": skill_match.get("score", 0) if isinstance(skill_match, dict) else 0,
                    "strongest": assessment.get("strongest_match", ""),
                    "biggest_gap": assessment.get("biggest_gap", ""),
                })

    asyncio.run(_process_all())

    if not results:
        click.echo("\nNo successful assessments.")
        return
    ...
    # Print summary table
    click.echo(f"\n{'=' * 100}")
    click.echo(f"BATCH ASSESSMENT RESULTS ({len(results)}/{len(urls)} successful)")
    click.echo(f"{'=' * 100}")
    click.echo(f"{'Grade':<6} {'%':>5} {'Company':<20} {'Title':<35} {'Strongest Match':<30}")
    click.echo(f"{'-' * 100}")
    for r in results:
        click.echo(
            f"{r['grade']:<6} {r['percentage']:>5.1f} "
            f"{r['company'][:19]:<20} "
            f"{r['title'][:34]:<35} "
            f"{r['strongest'][:29]:<30}"
        )
    ...
    top = results[0]
    click.echo(
        f"\nTop match: {top['company']} — {top['title']} "
        f"({top['percentage']:.0f}% {top['grade']})"
    )
